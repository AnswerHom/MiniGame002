...
const player = {
    ...
    levelUp() {
        this.level++;
        if (this.level < 5) { this.maxHp = 100 + (this.level - 1) * 20; this.attack = 10 + (this.level - 1) * 2; }
        else if (this.level < 10) { this.maxHp = 180 + (this.level - 5) * 25; this.attack = 18 + (this.level - 5) * 3; }
        else if (this.level < 15) { this.maxHp = 280 + (this.level - 10) * 30; this.attack = 28 + (this.level - 10) * 4; }
        else { this.maxHp = 380 + (this.level - 15) * 35; this.attack = 38 + (this.level - 15) * 5; }
        this.hp = this.maxHp;
    ...
    attackTarget(target) {
        if (this.attackCooldown <= 0) {
            ...
            for (let i = 0; i < 5; i++) createParticle(target.x + target.width / 2, target.y + target.height / 2, '#fff', 3);
        }
    },
    ...
    draw() {
        ...
        if (this.attacking) {
            ...
            ctx.fillStyle = '#e0e0e0'; ctx.fillRect(0, -3, 40, 6); ctx.fillStyle = '#8b4513'; ctx.fillRect(-5, -3, 10, 6);
        } else {
            ctx.translate(screenX + this.width / 2, this.y - this.height / 2);
            ctx.fillStyle = '#e0e0e0'; ctx.fillRect(15, -20, 6, 30); ctx.fillStyle = '#8b4513'; ctx.fillRect(12, -3, 10, 6);
        }
...
class Enemy {
    ...
    takeDamage(damage) {
        ...
        createFloatingText(this.x + this.width/2, this.y - this.height, '-' + Math.floor(damage), '#ff4444');
    ...
    die() {
        ...
        for (let i = 0; i < 10; i++) createParticle(this.x + this.width/2, this.y - this.height/2, this.color, 5);
    }
...
class Projectile {
    constructor(x, y, speed, damage, type) {
        ...
        this.width = 30; this.height = 8; this.alive = true; this.trail = [];
    }
    ...
    update(dt) {
        ...
        game.enemies.forEach(enemy => { if (!enemy.alive) return; if (this.x > enemy.x && this.x < enemy.x + enemy.width && this.y > enemy.y - enemy.height && this.y < enemy.y) { enemy.takeDamage(this.damage); this.alive = false; } });
        if (this.x > player.x + 500) this.alive = false;
    }
    
    draw() {
        this.trail.forEach(t => { const screenX = t.x - CONFIG.cameraOffset; ctx.globalAlpha = t.life; ctx.fillStyle = '#00ffff'; ctx.fillRect(screenX, t.y - 2, 20, 4); });
        ctx.globalAlpha = 1;
        const screenX = this.x - CONFIG.cameraOffset;
        ctx.fillStyle = '#00ffff'; ctx.fillRect(screenX, this.y - this.height/2, this.width, this.height);
...
function update(dt) {
    ...
    game.expOrbs.forEach(orb => orb.update(dt)); game.expOrbs = game.expOrbs.filter(orb => !orb.collected);
    game.particles.forEach(p => p.update(dt)); game.particles = game.particles.filter(p => p.life > 0);
    game.projectiles.forEach(p => p.update(dt)); game.projectiles = game.projectiles.filter(p => p.alive);
    game.clouds.forEach(cloud => { cloud.x += cloud.speed * dt; if (cloud.x > player.x + CONFIG.width) cloud.x = player.x - cloud.width; });
    game.stars.forEach(star => star.twinkle += star.speed * dt);
    game.grass.forEach(grass => grass.sway += dt * 2);
