...
function drawUI() {
    ...
    if (game.saveOpen) {
        ...
        ctx.font = 'bold 24px Microsoft YaHei';
        ctx.textAlign = 'center';
        ctx.fillText('📂 存档系统', CONFIG.width/2, 50);
        ...
        slots.forEach((slot, index) => {
            const isSelected = game.currentSaveSlot === slot.slot;
            const meta = slot.metadata;
            
            // 槽位背景
            ctx.fillStyle = isSelected ? '#334455' : '#222233';
            ctx.fillRect(CONFIG.width/2 - 200, slotY - 30, 400, 80);
            ctx.strokeStyle = isSelected ? '#00ff00' : '#444466';
            ctx.lineWidth = isSelected ? 2 : 1;
            ctx.strokeRect(CONFIG.width/2 - 200, slotY - 30, 400, 80);
            
            // 槽位标题
            ctx.fillStyle = '#ffffff';
            ctx.font = 'bold 16px Microsoft YaHei';
            ctx.textAlign = 'left';
            ctx.fillText(`槽位 ${slot.slot}`, CONFIG.width/2 - 180, slotY);
            
            if (meta) {
                ctx.fillStyle = '#aaaaaa';
                ctx.font = '12px Microsoft YaHei';
                ctx.fillText(`${meta.playerName} | Lv.${meta.level} ${meta.realm}`, CONFIG.width/2 - 180, slotY + 20);
                ctx.fillText(`游玩时间: ${Math.floor(meta.playTime/60)}分钟 | 保存时间: ${new Date(meta.savedAt).toLocaleString()}`, CONFIG.width/2 - 180, slotY + 38);
            } else {
                ctx.fillStyle = '#666666';
                ctx.font = 'italic 14px Microsoft YaHei';
                ctx.fillText('(空存档)', CONFIG.width/2 - 180, slotY + 20);
            }
        ...
        ctx.font = '14px Microsoft YaHei';
        ctx.textAlign = 'center';
        ctx.fillText(`当前选择: 槽位 ${game.currentSaveSlot}`, CONFIG.width/2, slotY + 10);
        
        ctx.fillStyle = '#ffff00';
        ctx.font = '12px Microsoft YaHei';
        ctx.fillText('快捷键: [1/2/3] 选择槽位 | [S] 保存 | [L] 读取 | [T] 查看信息 | [M] 关闭', CONFIG.width/2, slotY + 40);
    }
    
    // v1.12.0 新手引导界面
    if (game.showGuide && guideState.active) {
        ...
        ctx.textAlign = 'center';
        ctx.fillText(step.title, CONFIG.width/2, panelY + 40);
        ...
        let contentY = panelY + 80;
        lines.forEach((line, i) => {
            if (line.trim()) {
                ctx.fillText(line.trim() + (i < lines.length - 1 ? '。' : ''), CONFIG.width/2, contentY);
                contentY += 28;
            }
        });
        ...
        ctx.font = '12px Microsoft YaHei';
        ctx.fillText(`(${guideState.currentStep + 1}/${GUIDE_STEPS.length})`, CONFIG.width/2, panelY + 180);
        
        // 按钮
        const btnWidth = 120;
        const btnHeight = 36;
        const btnX = (CONFIG.width - btnWidth) / 2;
        const btnY = panelY + panelHeight - 55;
        ...
        ctx.font = 'bold 16px Microsoft YaHei';
        ctx.textAlign = 'center';
        ctx.fillText(step.button, CONFIG.width/2, btnY + 25);
        
        // 跳过提示
        ctx.fillStyle = '#666666';
        ctx.font = '11px Microsoft YaHei';
        ctx.fillText('按H跳过引导', CONFIG.width/2, panelY + panelHeight - 15);
    }
}
...
document.addEventListener('keydown', function(e) {
    ...
    // v1.12.0 存档系统 - M键打开存档界面
    if (e.code === 'KeyM' || e.key === 'm' || e.key === 'M') {
        game.saveOpen = !game.saveOpen;
        if (game.saveOpen) {
            game.alchemyOpen = false;
            game.companionOpen = false;
            game.sectionOpen = false;
            game.mountOpen = false;
            game.runeOpen = false;
            createFloatingText(player.x, player.y - 80, '📂 存档界面已打开', '#00ff00');
        }
    }
    ...
    if (game.saveOpen) {
        ...
        if (e.code === 'Digit1' || e.key === '1') {
            ...
            createFloatingText(player.x, player.y - 60, '选择槽位 1', '#00ff00');
        } else if (e.code === 'Digit2' || e.key === '2') {
            game.currentSaveSlot = 2;
            createFloatingText(player.x, player.y - 60, '选择槽位 2', '#00ff00');
        } else if (e.code === 'Digit3' || e.key === '3') {
            game.currentSaveSlot = 3;
            createFloatingText(player.x, player.y - 60, '选择槽位 3', '#00ff00');
        }
        // S键保存
        if (e.code === 'KeyS' || e.key === 's' || e.key === 'S') {
            saveGame(game.currentSaveSlot);
        }
        // L键加载
        if (e.code === 'KeyL' || e.key === 'l' || e.key === 'L') {
            if (loadGame(game.currentSaveSlot)) {
                // 重新初始化游戏循环
                game.lastTime = performance.now();
            }
        }
        ...
        if (e.code === 'KeyT' || e.key === 't' || e.key === 'T') {
            const meta = getSaveMetadata(game.currentSaveSlot);
            if (meta) {
                createFloatingText(player.x, player.y - 60, `存档: ${meta.level}级 ${meta.realm} 游玩${Math.floor(meta.playTime/60)}分钟`, '#00ffff');
            } else {
                createFloatingText(player.x, player.y - 60, '该槽位暂无存档', '#ff8800');
            }
        }
    }
    
    // v1.12.0 新手引导 - H键重新显示引导
    if (e.code === 'KeyH' || e.key === 'h' || e.key === 'H') {
        if (!guideState.completed) {
            startGuide();
            createFloatingText(player.x, player.y - 80, '📖 打开新手引导', '#00ff00');
        } else {
            // 重新开始引导
            resetGuide();
            createFloatingText(player.x, player.y - 80, '📖 重新开始引导', '#00ff00');
        }
    }
    
    // v1.12.0 引导界面操作 - Enter/Space 确认下一步
    if (game.showGuide && guideState.active) {
